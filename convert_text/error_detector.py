...
class ErrorDetector:
    ...
    def _generate_correction_report(self, results: List[Dict], input_file: str) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("=" * 70 + "\n")
            f.write("Automatic Correction Report for Speech Transcription (Batch-Optimized)\n")
            f.write("=" * 70 + "\n")
            f.write(f"Input file: {input_file}\n")
            f.write(f"Processed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Total segments: {total_segments}\n")
            f.write(f"Corrected segments: {corrected_segments}\n")
            f.write(f"Correction rate: {corrected_segments/total_segments*100:.2f}%\n")
            if api_errors > 0:
                f.write(f"API errors: {api_errors}\n")

            f.write("\nProcessing method distribution:\n")
            f.write(f"  Batch API processing: {batch_api_count} ({batch_api_count/total_segments*100:.1f}%)\n")
            f.write(f"  Quick fix: {quick_fix_count} ({quick_fix_count/total_segments*100:.1f}%)\n")
            f.write(f"  Pre-filter skipped: {pre_filter_count} ({pre_filter_count/total_segments*100:.1f}%)\n")
            ...
            for i, result in enumerate(results, 1):
                ...
                if 'error' in result:
                    ...
                    f.write("🔧 Corrected\n")
                    f.write(f"Original: {result.get('original_text', result.get('text', ''))}\n")
    ...
    def _print_correction_summary(self, results: List[Dict]):
        ...
        print(f"Pre-filter skipped: {pre_filter_count} ({pre_filter_count/total*100:.1f}%)")
        print("=" * 50)
    ...
    def detect_and_correct_file_only_correct(self, input_file: str) -> str:
        ...
        print("Starting error detection and auto-correction (only generate corrected file)...")
